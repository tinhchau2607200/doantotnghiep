<?php
	include("../connect.php"); //Kết nối đến CSDL
	mysql_query("SET NAMES utf8");
	//Lấy dữ liệu
	if ($_POST['datban']!="")
	{
		$ten=$_POST['ten'];
		$sdt=$_POST['sdt'];
		$email=$_POST['email'];
		$ngaydat=date('Y-m-d', strtotime($_POST['ngaydat']));
		$idphong=$_POST['phong'];
		$sql="INSERT INTO datban(ten,sdt,email,ngaydat,idphong) VALUES ('$ten','$sdt','$email','$ngaydat','$idphong')";
		mysql_query($sql);
		header("location:../../index.php?ac=datban");
	}	
	elseif($_POST['suabaiviet']!="")
	{
		//Thực hiện sửa loại tin
		$ten=$_POST['ten'];
		$name=$_FILES['anhminhhoa']['name'];//Lấy tên của tệp tin ảnh
		if($name!='')
		{
			$time=date("Ymdhis");
			$name=$time."_".$name;//Nối thêm ngày và thời gian vào tên file ảnh
			$dich="../../../images/uploads/".$name;// đường dẫn file ảnh 
			copy($_FILES['anhminhhoa']['tmp_name'],$dich); //Sao chép file ảnh đến đường dẫn đích
			$dich=substr($dich,9);//Cắt bớt 9 ký tự đầu trong đường dẫn
		}
		$sdt=$_POST['sdt'];
		$email=$_POST['email'];
		$idloaitin=$_POST['loaitin'];
		//Định nghĩa câu lệnh sửa
		$sql="UPDATE baiviet SET ten='$ten', anhminhhoa='$dich', sdt='$sdt',email='$email',idloaitin='$idloaitin' WHERE idbaiviet='$_GET[id]'";
		//Thực hiện câu lệnh đã định nghĩa
		mysql_query($sql);
		header("location: ../../index.php?ac=tintuc&id=$_GET[id]");
	}
	else
	{
		//Thực hiện xóa bài viết
		$sql="delete from datban where iddatban='$_GET[id]'";
		mysql_query($sql);
		header("location: ../../index.php?ac=datban");
	}
?>